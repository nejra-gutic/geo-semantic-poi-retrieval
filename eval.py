...
def main():
    ...
    def embeddings_with_boost(query):
    ...
    def rrf_with_boost(query):
        ...
        rrf_k = 60
        scores = {}
        for rank, poi_id in enumerate(bm25_results["poi_id"].tolist(), start=1):
            scores[poi_id] = scores.get(poi_id, 0) + 1 / (rrf_k + rank)
        for rank, poi_id in enumerate(emb_results["poi_id"].tolist(), start=1):
            scores[poi_id] = scores.get(poi_id, 0) + 1 / (rrf_k + rank)
        rrf_df = pd.DataFrame({
