...
def search_bm25(
    ...
    if df_filtered is not None:
        corpus = df_filtered["poi_text_lemma"].fillna("").apply(
            lambda x: [t for t in x.split() if len(t) > 1]
        ).tolist()
        bm25_filtered = BM25Okapi(corpus)
        scores = bm25_filtered.get_scores(tokens)
        candidate_k = min(len(scores), top_k * 10)
    ...
    results = results.drop_duplicates(subset=["name"]).head(top_k)
